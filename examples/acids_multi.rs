...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for run in &multi_runs {
        for (s, name) in species_names.iter().enumerate().take(n_species) {
            println!(
                "{:<12} {:<14} {:>12.6} {:>12.1}",
                name, run.solver_name, run.peak_concentrations[s], run.retention_times[s]
            );
        }
        println!("{:-<55}", "");
    }
    ...
    for run in &multi_runs {
        println!(
            "{:<12} {:<14} {:>12.3}",
            "Competitive", run.solver_name, run.elapsed_secs
        );
    }
    ...
    for run in &solo_runs {
        let is_peak =
            run.peak_concentration > 1e-4 && run.final_concentration < run.peak_concentration * 0.1;
        ...
        println!(
            "  {:<12} {:<14} : {}",
            run.species_name, run.solver_name, status
        );
    }
    ...
    for run in &multi_runs {
        for s in 0..n_species {
            ...
            println!(
                "  {:<12} {:<14} : {}",
                species_names[s], run.solver_name, status
            );
        }
    }
