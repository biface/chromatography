...
### Changed
- `Cargo.toml` — added `[profile.release]` (`strip = true`, `lto = "thin"`, `codegen-units = 1`) — not previously tuned, building with Cargo's untouched defaults; needed before packaging a distributable binary
- `Cargo.toml` — `plotters`' default `ttf` feature (→ `font-kit` → `fontconfig`/`freetype`, a real dynamic runtime dependency on Linux, confirmed via `cargo tree -e normal -i font-kit`: `chrom-rs → plotters → font-kit`, not a `criterion`/dev-only artifact) swapped for `ab_glyph`, a pure-Rust font renderer with zero system dependency — see `binary-distribution-plan.md`, Variant B (the only variant built for this release). `default-features = false` with the same feature set `plotters`' own `default` provides, minus `ttf`, plus `ab_glyph`.
- `assets/fonts/DejaVuSans.ttf` (new, Bitstream Vera Fonts license, see `assets/fonts/LICENSE-DEJAVU.txt` — named distinctly from the crate's own top-level `LICENSE-APACHE` to avoid any ambiguity about which license governs what) embedded via `include_bytes!` and registered under the `"sans-serif"` family — the name every chart in this crate uses — through the new `chrom_rs::output::register_fonts()` (`src/output/visualization/fonts.rs`). Called at the top of `main()` in `src/main.rs` and in each of the 5 `tools/plot_*.rs` binaries (font registration is process-global, not shared across binaries). Only `FontStyle::Normal` is registered; `plotters` falls back to `Normal` for any unregistered style under the same family, and no chart in this crate requests Bold/Oblique today.
